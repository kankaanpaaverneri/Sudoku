...
public class SudokuManager {
    ...
    public static boolean setSudokuValue(
        final int numberAttempt,
        final int column,
        final int row
    ) {
        ...
        if (
            scanResult == ScanResult.NOT_FOUND || scanResult == ScanResult.ZERO
        ) isNumberValid = true;
        else isNumberValid = false;
    ...
    private boolean isGridValid() {
        for (int column = 0; column < getHeight(); column++) {
            for (int row = 0; row < getWidth(); row++) {
                if (
                    isNumberValid(grid[column][row], column, row) !=
    ...
    private boolean addNumbersToGrid() {
        for (int column = 0; column < getHeight(); column++) {
            ...
            for (int row = 0; row < getWidth();) {
                ...
                final int numberAttempt = nineUniqueNumbers.get(
                    uniqueNumbersIndex
                );
                if (
                    isNumberValid(numberAttempt, column, row) ==
                    ScanResult.NOT_FOUND
                ) {
                    ...
                    if (swapNumberStackFrameCounter > 100) return false;
                }
            }
        }
        return true;
    ...
    private static void swapNumbersInGrid(
        ...
        if (scanRow(numberAttempt, column) == ScanResult.ROW) {
            ...
            newPosition = moveCurrentPosition(
                numberAttempt,
                column,
                row,
                ScanResult.ROW
            ...
            newNumberOption = getNumberOptions(
    ...
    private static Position moveCurrentPosition(
        final int numberAttempt,
        final int column,
        final int row,
        ...
        if (scanResult == ScanResult.ROW) {
            for (int i = 0; i < getWidth(); i++) {
                if (grid[column][i] == numberAttempt) return new Position(
                    column,
                    i
                );
            }
        ...
        if (scanResult == ScanResult.COLUMN) {
            for (int i = 0; i < getHeight(); i++) {
                if (grid[i][row] == numberAttempt) return new Position(i, row);
            }
        }
        if (scanResult == ScanResult.BOX) {
            ...
            for (
                int i = columnStart;
                i < columnStart + (getHeight() / 3);
                i++
            ) {
                for (int j = rowStart; j < rowStart + (getHeight() / 3); j++) {
                    if (grid[i][j] == numberAttempt) return new Position(i, j);
                }
            }
        }
    ...
    private static int getNumberOptions(
        final int column,
        final int row,
        ScanResult scanResult
    ) {
        ...
        if (scanResult == ScanResult.ROW) {
            for (int i = 0; i < getWidth(); i++) {
                for (int u = 0; u < unusedNumbers.getSize(); u++) {
                    if (
                        unusedNumbers.get(u) == grid[column][i] &&
                        unusedNumbers.get(u) != 0
        ...
        if (scanResult == ScanResult.COLUMN) {
            for (int i = 0; i < getHeight(); i++) {
                for (int u = 0; u < unusedNumbers.getSize(); u++) {
                    if (
                        unusedNumbers.get(u) == grid[i][row] &&
                        unusedNumbers.get(u) != 0
                    ) unusedNumbers.set(u, 0);
                }
            }
    ...
    private static ScanResult isNumberValid(
        final int numberAttempt,
        final int column,
        final int row
    ) {
        //This one return an enums ROW, COLUMN or BOX if there is a conflicting number
        if (numberAttempt == 0) return ScanResult.ZERO;

        if (
            scanRow(numberAttempt, column) == ScanResult.ROW
        ) return ScanResult.ROW;
        if (
            scanColumn(numberAttempt, row) == ScanResult.COLUMN
        ) return ScanResult.COLUMN;
        if (
            scanBox(numberAttempt, column, row) == ScanResult.BOX
        ) return ScanResult.BOX;

        return ScanResult.NOT_FOUND;
    }

    private static ScanResult scanColumn(
        final int numberAttempt,
        final int row
    ) {
        for (int i = 0; i < getHeight(); i++) {
    ...
    private static ScanResult scanRow(
        final int numberAttempt,
        final int column
    ) {
        for (int i = 0; i < getWidth(); i++) {
    ...
    private static ScanResult scanBox(
        final int numberAttempt,
        final int column,
        final int row
    ) {
        ...
        for (int i = columnStart; i < columnStart + 3; i++) {
            for (int j = rowStart; j < rowStart + 3; j++) {
                if (numberAttempt == grid[i][j]) return ScanResult.BOX;
            }
        }
        return ScanResult.NOT_FOUND;
    }
    ...
    public static boolean isSudokuSolved() {
        for (int column = 0; column < getHeight(); column++) {
            for (int row = 0; row < getWidth(); row++) {
                int temp = grid[column][row];
                grid[column][row] = 0;
                if (isNumberValid(temp, column, row) != ScanResult.NOT_FOUND) {
